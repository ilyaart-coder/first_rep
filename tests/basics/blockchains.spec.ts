...
// GET /basics/blockchains/ с авторизацией
async function getBlockchains(request: APIRequestContext, params: Record<string, string>) {
  const tokens = getTokensOrThrow();
  const search = new URLSearchParams(params);
  const url = `${env.apiUrl}/basics/blockchains/?${search.toString()}`;
...
test.describe('Basics /basics/blockchains/', () => {
  // Базовая проверка структуры ответа
  test('возвращает список блокчейнов с корректной структурой', async ({ request }) => {
    const response = await getBlockchains(request, { page: '1', size: '25' });
  ...
  test('пагинация size=1 возвращает максимум 1 элемент', async ({ request }) => {
    const response = await getBlockchains(request, { page: '1', size: '1' });
    expect(response.status()).toBe(200);
  ...
  // search по коду блокчейна (если есть результаты)
  test('search фильтрует по коду блокчейна', async ({ request }) => {
    const baseResponse = await getBlockchains(request, { page: '1', size: '25' });
    expect(baseResponse.status()).toBe(200);
    const baseBody = (await baseResponse.json()) as BasicsNetworksResponse;
    const first = baseBody.results?.[0];
    if (!first?.code) test.skip(true, 'Нет блокчейнов для проверки search');

    const search = first!.code!.slice(0, 2);
    const response = await getBlockchains(request, { page: '1', size: '25', search });
    expect(response.status()).toBe(200);
